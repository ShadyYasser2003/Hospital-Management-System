...
const getTypeConfig = (type: string) => {
  switch (type?.toUpperCase()) {
    ...
    case 'RADIOLOGY_ORDER_COMPLETED':
      return { icon: <FlaskConical className="h-5 w-5" />, color: 'text-violet-600', bg: 'bg-violet-50 dark:bg-violet-950' };
    case 'BLOOD_REQUEST_CREATED':
    case 'BLOOD_REQUEST_RESERVED':
    case 'BLOOD_REQUEST_COMPLETED':
    case 'BLOOD_REQUEST_CANCELLED':
      return { icon: <AlertTriangle className="h-5 w-5" />, color: 'text-red-600', bg: 'bg-red-50 dark:bg-red-950' };
    case 'TRANSFER_REQUEST_CREATED':
    case 'TRANSFER_REQUEST_SENT':
    case 'TRANSFER_REQUEST_FAILED':
      return { icon: <Info className="h-5 w-5" />, color: 'text-orange-600', bg: 'bg-orange-50 dark:bg-orange-950' };
    case 'SYSTEM_ALERT':
