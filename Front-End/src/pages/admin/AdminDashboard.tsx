...
import { inventorySummary, daysUntilExpiry } from '@/lib/bloodBankUtils';
import { Users, Building, Calendar, Bed, Pill, UserPlus, Bell, Droplets } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const { data: bloodReqs = [] }     = useBloodRequests();
  ...
  const bloodSummary      = inventorySummary(bloodUnits);
  const totalBloodUnits   = Object.values(bloodSummary).reduce((a, b) => a + b, 0);
  const pendingBloodReqs  = bloodReqs.filter(r => r.status === 'PENDING').length;
  const expiringSoon      = bloodUnits.filter(u => {
    const d = daysUntilExpiry(u.expiryDate);
    return d !== null && d >= 0 && d <= 7 && u.status === 'AVAILABLE';
  }).length;
  ...
  return (
    <DashboardLayout navItems={adminNavItems} title="Admin Dashboard">
      ...
      <h2 className="text-lg font-semibold mb-4">Blood Bank</h2>
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        <StatCard title="Blood Units in Stock" value={String(totalBloodUnits)}   icon={<Droplets className="h-6 w-6" />} variant="primary" />
        <StatCard title="Pending Requests"     value={String(pendingBloodReqs)}  icon={<Droplets className="h-6 w-6" />} variant="warning" />
        <StatCard title="Expiring ≤ 7 days"    value={String(expiringSoon)}      icon={<Droplets className="h-6 w-6" />} variant="accent" />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <QuickAction title="Blood Bank"     icon={<Droplets className="h-5 w-5" />}  to="/admin/blood-bank" />
      </div>
    </DashboardLayout>
  );
};
