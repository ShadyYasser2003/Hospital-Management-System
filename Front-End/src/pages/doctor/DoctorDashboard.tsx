...
import { useDiagnosticOrdersByDoctor } from '@/hooks/useDiagnosticOrders';
import { useUnreadCount } from '@/hooks/useNotifications';
import { usePatients } from '@/hooks/usePatients';
import { LayoutDashboard, Calendar, Users, FileText, Pill, ClipboardList, User, Bed, TestTube, Bell, ArrowRightLeft, Droplets } from 'lucide-react';
...
export const doctorNavItems = [
  ...
  { label: 'Blood Requests',  path: '/doctor/blood-bank',    icon: <Droplets className="h-5 w-5" /> },
  { label: 'Admissions',      path: '/doctor/admissions',    icon: <Bed className="h-5 w-5" /> },
  { label: 'Transfers',       path: '/doctor/transfers',     icon: <ArrowRightLeft className="h-5 w-5" /> },
...
export const DoctorDashboard: React.FC = () => {
  ...
  const { pending: pendingTests }    = useDiagnosticOrdersByDoctor(user?.id);
