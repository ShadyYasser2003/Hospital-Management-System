...
const prescriptionService = {
  ...
  async getByDoctor(doctorId: number | string): Promise<PrescriptionDto[]> {
    const { data } = await api.get<PrescriptionDto[]>(`/api/prescriptions/doctor/${doctorId}`);
    return data;
  },
  ...
  async dispense(id: number | string): Promise<PrescriptionDto> {
    const { data } = await api.put<PrescriptionDto>(`/api/prescriptions/${id}/dispense`);
    return data;
  },

  async cancelPrescription(id: number | string): Promise<PrescriptionDto> {
    const { data } = await api.put<PrescriptionDto>(`/api/prescriptions/${id}/cancel`);
    return data;
  },
};
