  ...
  const invoiceCols = [
    ...
    { key: 'actions', header: 'Actions', render: (i: InvoiceDto) => (
      <div className="flex gap-1">
        <Button variant="ghost" size="sm" onClick={() => { setSelectedInvoice(i); setDetailOpen(true); }}>
          <Eye className="h-4 w-4" />
        </Button>
        {i.status?.toUpperCase() !== 'PAID' && i.status?.toUpperCase() !== 'CANCELLED' && (
          <Button variant="ghost" size="sm" className="text-destructive" onClick={() => handleCancel(i)}>✕</Button>
        )}
      </div>
    )},
  ];
        ...
        <Tabs defaultValue="unpaid">
          ...
          <TabsContent value="unpaid">
            <Card><CardContent className="pt-6">
              <DataTable data={unpaid} columns={invoiceCols} emptyMessage="No unpaid invoices" />
            </CardContent></Card>
          </TabsContent>
          <TabsContent value="paid">
            <Card><CardContent className="pt-6">
              <DataTable data={paid} columns={invoiceCols} emptyMessage="No paid invoices" />
            </CardContent></Card>
          </TabsContent>
          <TabsContent value="all">
            <Card><CardContent className="pt-6">
              <DataTable data={allPage?.content ?? []} columns={invoiceCols} emptyMessage="No invoices" />
            </CardContent></Card>
          </TabsContent>
        </Tabs>
      )}
