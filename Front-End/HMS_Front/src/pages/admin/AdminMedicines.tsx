...
const AdminMedicines = () => {
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      const { id, ...payload } = editingMed;
      await updateMedicine.mutateAsync({ id, payload });
  ...
  const columns = [
    ...
    { key: 'genericName',          header: 'Generic Name',         render: (m: MedicineDto) => m.genericName || '—' },
    { key: 'prescriptionRequired', header: 'Prescription',         render: (m: MedicineDto) => m.prescriptionRequired ? <span className="text-xs font-medium text-orange-600 bg-orange-50 px-2 py-0.5 rounded">Required</span> : <span className="text-xs text-muted-foreground">OTC</span> },
    { key: 'sideEffects',          header: 'Side Effects',         render: (m: MedicineDto) => m.sideEffects ? <span className="text-sm text-muted-foreground">{m.sideEffects.substring(0, 40)}{m.sideEffects.length > 40 ? '…' : ''}</span> : '—' },
    { key: 'description',          header: 'Description',          render: (m: MedicineDto) => m.description ? <span className="text-sm text-muted-foreground">{m.description.substring(0, 40)}{m.description.length > 40 ? '…' : ''}</span> : '—' },
    { key: 'status',               header: 'Status',               render: (m: MedicineDto) => <StatusBadge status={m.status?.toLowerCase() as never} /> },
    {
      key: 'actions', header: 'Actions',
      render: (m: MedicineDto) => (
        <div className="flex gap-1">
          <Button variant="ghost" size="sm" onClick={() => handleOpenEdit(m)}><Edit className="h-4 w-4" /></Button>
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={() => handleDelete(m.id)} className="bg-destructive hover:bg-destructive/90">Delete</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>
  ...
  return (
    <DashboardLayout navItems={adminNavItems} title="Medicine Stock">
      ...
      {!isLoading && !error && <DataTable data={filtered} columns={columns} emptyMessage="No medicines found" />}
    </DashboardLayout>
  );
};
