...
// ── Constants (defined once, outside every component) ─────────────────────────

const WELCOME_MESSAGE: Message = {
  id: 'welcome',
  role: 'ai',
  content:
    'مرحباً! أنا مساعدك الطبي الذكي. يمكنني مساعدتك في فهم الأعراض، والاستفسار عن المواعيد، أو تقديم معلومات عن الأدوية. كيف يمكنني مساعدتك اليوم؟',
};
...
const ChatHeader = React.memo<HeaderProps>(({ isFullScreen, onClose, onToggleFullScreen }) => (
  <div
    className={cn(
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className={cn('text-primary-foreground/75 leading-tight', isFullScreen ? 'text-sm' : 'text-xs')}>
          اسألني عن الأعراض أو الأدوية أو المواعيد
        </p>
      </div>
    </div>
...
const ChatFooter = React.memo<FooterProps>(({
  inputValue, isLoading, isFullScreen, inputRef, onChange, onKeyDown, onSend,
}) => (
  <>
    <div className={cn(
