...
public class MedicineServiceImpl implements MedicineService {
    ...
    public MedicineDto createNewMedicine(MedicineDto medicineDto) {
        // Name is required
        if (medicineDto.getName() == null || medicineDto.getName().isBlank()) {
            throw new RuntimeException("Medicine name is required");
        }
        String trimmedName = medicineDto.getName().trim();
        if (repository.findByNameIgnoreCase(trimmedName).isPresent()) {
            throw new RuntimeException("Name already taken, choose another unique name");
        }

        String genericName = (medicineDto.getGenericName() != null && !medicineDto.getGenericName().isBlank())
                ? medicineDto.getGenericName().trim()
                : trimmedName;

        if (repository.findByGenericNameIgnoreCase(genericName).isPresent()) {
            throw new RuntimeException("Generic name already taken, choose another unique name");
        }

        medicineDto.setName(trimmedName);
    ...
    public MedicineDto updateMedicine(Long id, MedicineDto medicineDto) {
        ...
        if (medicineDto.getName() != null) {
            ...
            if (!newName.equalsIgnoreCase(existingMedicine.getName())) {
                repository.findByNameIgnoreCase(newName).ifPresent(other -> {
                    if (!other.getId().equals(id)) {
                        throw new RuntimeException("Name already used, please pick another different but unique name");
                    }
                });
                existingMedicine.setName(newName);
            }
        }
        if (medicineDto.getGenericName() != null) {
            String newGeneric = medicineDto.getGenericName().isBlank()
                    ? existingMedicine.getGenericName()
                    : medicineDto.getGenericName().trim();
            if (newGeneric != null && !newGeneric.equalsIgnoreCase(existingMedicine.getGenericName())) {
                repository.findByGenericNameIgnoreCase(newGeneric).ifPresent(other -> {
                    if (!other.getId().equals(id)) {
                        throw new RuntimeException("Generic name already used, please pick another different but unique name");
                    }
                });
                existingMedicine.setGenericName(newGeneric);
            }
        }
