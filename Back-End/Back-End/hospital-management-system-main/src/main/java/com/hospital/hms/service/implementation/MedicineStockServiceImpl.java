...
public class MedicineStockServiceImpl implements MedicineStockService {
    ...
    @Override
    public MedicineStockDto updateMedicineStockFull(Long id, MedicineStockDto dto) {
        MedicineStock stock = repository.findById(id)
                .orElseThrow(() -> new RuntimeException("Medicine Stock not found with id: " + id));
        if (dto.getManufacturer() != null)     stock.setManufacturer(dto.getManufacturer());
        if (dto.getReOrderLevel() != null)     stock.setReOrderLevel(dto.getReOrderLevel());
        if (dto.getSellingPrice() != null)     stock.setSellingPrice(dto.getSellingPrice());
        if (dto.getUnitPurchase() != null)     stock.setUnitPurchase(dto.getUnitPurchase());
        if (dto.getMedicineForm() != null)     stock.setMedicineForm(dto.getMedicineForm());
        if (dto.getExpiryDate() != null)       stock.setExpiryDate(com.hospital.hms.config.DateUtils.parse(dto.getExpiryDate()));
        if (dto.getStorageLocation() != null)  stock.setStorageLocation(dto.getStorageLocation());
        if (dto.getDosage() != null)           stock.setDosage(dto.getDosage());
        if (dto.getPackageSize() != null)      stock.setPackageSize(dto.getPackageSize());
        if (dto.getCurrentQuantity() != null)  stock.setCurrentQuantity(dto.getCurrentQuantity());
        if (dto.getPurchaseQuantity() != null) stock.setPurchaseQuantity(dto.getPurchaseQuantity());
        return MedicineStockMapper.mapToDto(repository.save(stock));
    }
    ...
    public List<MedicineStockDto> getExpiringMedicineStock(int days) {
        ...
        for(MedicineStock stock:stockList){
            ...
            // Include stocks that expire between now and threshold (not yet expired)
            if(expiryDate != null && expiryDate.isAfter(now) && !expiryDate.isAfter(threshold)){
